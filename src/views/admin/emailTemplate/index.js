...
export default function EmailTemp() {
    ...
    const [success, setSuccess] = useState(); // Keep track of the content of the selected tab

    const handleOpen = async () => {
        try {
            const response = await adminAxios.get(`/email-templates`, { token });
    ...
    const handleChange = (event, newValue) => {
        ...
        setId(emailData[newValue]?.id || null); // Set the content of the selected tab
    };
                    ...
                    {loading ? (
                        ...
                        <TabPanel value={value} index={value}>
                            {content !== null ? ( // Check if content is available before rendering TemplatePage
                                <TemplatePage id={id} row={content} />
                            ) : (
