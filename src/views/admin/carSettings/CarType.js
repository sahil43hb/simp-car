...
export default function Furnishing() {
    ...
    const handleStore = async () => {
        ...
        try {
            const response = await adminAxios.post('/add-type', { token, type });
            // console.log(response.data);
    ...
    const handleDelete = async (id) => {
        try {
            const response = await adminAxios.delete(`/delete-type/${id}`, { token });
            // console.log(response.data);
            if(response.data === 'success'){
                setOpen(true);
                handleOpen();
    ...
    return (
        <MainCard
            content={false} title="Car Type"
            >

            <Grid container spacing={2} sx={{ padding: '10px', justifyContent: 'end' }}>
            <Grid item xs={12} sm={4} md={3} >
            <Grid item >
            <TextField fullWidth variant='outlined' label="Add Car Type" value={type} onChange={(event)=>setType(event.target.value)} />
        </Grid>
            </Grid>
