...
export default function MonthTier() {
    const Time = [
        { label: 'Select a Value', id: 1 },
        { label: '3 Months', id: 1 },
        { label: '6 Months', id: 2 },
        { label: '12 Months', id: 3 },
        { label: '24 Months', id: 4 },
        { label: '36 Months', id: 5 }
    ];
    ...
    const handleDefault = async () => {
        try {
            const response = await adminAxios.get('/month', { token });
    ...
    const handleOpen = async () => {
        ...
        try {
            const response = await adminAxios.post('/add-month', { token, label: month.label});
            // console.log(response.data);
    ...
    const handleDelete = async (id) => {
        try {
            const response = await adminAxios.delete(`/delete-month/${id}`, { token });
            // console.log(response.data);
            if(response.data === 'success'){
                setOpen(true);
                handleDefault();
    ...
    return (
        <MainCard
            content={false} title="Monthly Tier"
        >
